...
Use the **`/sdd.plan`** command to create `plan.md` as the planning control plane. It captures Stage 0 shared context, the planning queue, and the binding registry ledger for later `sdd.plan.*` child commands.
...
`Binding Projection Index` is a registry-only lookup surface; binding semantics remain authoritative in `test-matrix.md` `Binding Packets`.
...
It should emit single-target work packages only: each task must carry one explicit path or command target plus one primary completion anchor. If required anchors are missing from `plan.md`, `contracts/`, or `test-matrix.md`, if a selected contract is `blocked`, or if contract hygiene blockers are present (for example missing `Full Field Dictionary (Operation-scoped)` or unresolved binding-packet source / packet-authority issues), `/sdd.tasks` should stop and route back to the relevant `/sdd.plan.*` command rather than inferring new semantics or emitting `blocked`/`todo` placeholder tasks.

The prerequisite script emits a compact runtime `TASKS_BOOTSTRAP` packet from `plan.md` so `/sdd.tasks` can reuse a joined control-plane inventory instead of reparsing the planning tables during the same run. If that runtime bootstrap cannot be produced, stop and repair the bootstrap path instead of recomputing the gate from mirrored helper logic.

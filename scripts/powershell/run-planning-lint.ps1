...
foreach ($row in $rows) {
    ...
    switch ($row.kind) {
        ...
        default {
            Add-Finding -RuleId $row.id -Severity $row.severity -File $row.glob -Line 0 -Message "Unsupported rule kind: $($row.kind)" -Remediation 'Use one of: file_regex_forbidden, file_regex_required_any, component_symbols_exist.'
        }
    }
}

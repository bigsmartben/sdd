...
- `/sdd.tasks` MUST NOT read or depend on `/sdd.plan` internal `handoff payload`; it consumes persisted planning artifacts only.
...
5. **Execute task generation workflow**:
   ...
   - Repository-first consumption discipline:
     ...
     - Do not derive dependency matrix / invocation governance semantics from `README.md`, `docs/**`, planning narratives, or generated summaries.
...
- Repository-first projection artifacts in `.specify/memory/repository-first/` are complementary and MUST NOT replace one another (`technical-dependency-matrix.md` = dependency facts, `module-invocation-spec.md` = invocation constraints).

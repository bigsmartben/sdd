...
|----------------|---------|-------------|
| **CLI Tool Only** | `uv tool install specify-cli --force --from git+https://github.com/bigsmartben/sdd.git` | Get latest CLI features without touching project files |
...
```bash
uvx --from git+https://github.com/bigsmartben/sdd.git specify init --here --ai copilot
```
...
Once you've run `specify init`, the slash commands (like `/sdd.specify`, `/sdd.plan`, etc.) are **permanently installed** in your project's agent folder (`.claude/`, `.github/prompts/`, etc.). Your AI assistant reads these command files directly—no need to run `specify` again.
...
- **Test new slash commands:** Run `/sdd.constitution` or another command to verify everything works
- **Review release notes:** Check [GitHub Releases](https://github.com/bigsmartben/sdd/releases) for new features and breaking changes

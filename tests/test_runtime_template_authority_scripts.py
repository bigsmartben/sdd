...
def assert_local_execution_protocol(payload: dict) -> None:
    ...
    if protocol["python"]["available"]:
        ...
        assert protocol["python"]["runner_cmd"] == "specify internal-run-python --script <helper-script> -- <helper-args>"
...
def test_check_prerequisites_powershell_accepts_command_name_override_for_specify(tmp_path):
    repo_dir = tmp_path / "repo"
    feature_dir = repo_dir / "specs" / "001-demo"
    feature_dir.mkdir(parents=True)
    (feature_dir / "spec.md").write_text("# Spec\n", encoding="utf-8")
    ...
    specify_cmd = bin_dir / "specify.cmd"
    specify_cmd.write_text(
        "@echo off\n"

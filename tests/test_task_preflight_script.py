...
def _write_minimal_feature(feature_dir: Path) -> None:
    ...
    (feature_dir / "interface-details" / "create-task.md").write_text("# Interface Detail\n", encoding="utf-8")
...
| BindingRowID-001 | interface-detail | `interface-details/create-task.md` | done | a | b | [none] |
""",
        encoding="utf-8",
    )


def test_task_preflight_helper_emits_joined_unit_inventory(tmp_path):
    ...
    result = subprocess.run(
        [
            ...
            "--interface-details-dir",
            str(feature_dir / "interface-details"),
        ],
        capture_output=True,
        text=True,
        check=False,
    ...
    assert payload["artifact_status_summary"]["interface-detail"]["done"] == 1
    ...
    assert unit["interface_detail"]["target_path"] == "interface-details/create-task.md"
    assert unit["interface_detail"]["exists"] is True


def test_task_preflight_helper_excludes_missing_artifacts_from_ready_inventory(tmp_path):
    feature_dir = tmp_path / "specs" / "001-demo"
    _write_minimal_feature(feature_dir)
    (feature_dir / "contracts" / "create-task.md").unlink()
    (feature_dir / "interface-details" / "create-task.md").unlink()
    ...
    assert payload["unit_inventory"][0]["interface_detail"]["exists"] is False
...
def test_bash_check_prerequisites_can_embed_tasks_bootstrap(tmp_path):
    ...
    result = subprocess.run(
        ["bash", str(scripts_bash / "check-prerequisites.sh"), "--json", "--task-preflight"],
        cwd=repo_dir,
        env=env,
        capture_output=True,
        text=True,
...
def test_bash_check_prerequisites_keeps_task_preflight_optional_when_helper_missing(tmp_path):
    repo_dir = tmp_path / "repo"
    scripts_bash = repo_dir / "scripts" / "bash"
    scripts_bash.mkdir(parents=True)
    ...
    assert result.returncode == 0, result.stderr
    payload = json.loads(result.stdout)
    assert payload["TASKS_BOOTSTRAP"] is None


def test_bash_check_prerequisites_keeps_task_preflight_optional_when_python_fails(tmp_path):
    repo_dir = tmp_path / "repo"
    scripts_bash = repo_dir / "scripts" / "bash"
    scripts_bash.mkdir(parents=True)
...
def test_tasks_command_prefers_task_preflight_bootstrap():
    ...
    interface_detail_template = (REPO_ROOT / "templates" / "interface-detail-template.md").read_text(encoding="utf-8")
    ...
    assert "`TASKS_BOOTSTRAP` is an optional derived run-local control-plane projection" in tasks_command
    assert "Prefer `TASKS_BOOTSTRAP.unit_inventory` and `TASKS_BOOTSTRAP.ready_unit_inventory`" in tasks_command
    assert "Build one unique run-local `tuple-index` from `TASKS_BOOTSTRAP.unit_inventory` when available" in tasks_command
    ...
    assert "-AsHashtable" not in powershell_script
    assert '$payload.TASKS_BOOTSTRAP = $null' in powershell_script

    assert "If `Boundary Anchor` and `Implementation Entry Anchor` resolve to the same repo-backed symbol, reuse one participant" in interface_detail_template
    assert "%% Declare Entry only when it differs from Boundary." in interface_detail_template
    assert 'participant Entry as "<ImplementationEntryAnchorWhenDistinct>"' not in interface_detail_template
    assert "Boundary->>Entry: handoff to internal entry (S2)" in interface_detail_template

...
class TestInstallAiSkills:
    ...
    def test_skills_installed_with_correct_structure(self, project_dir, templates_dir):
        ...
        assert "author: bigsmartben/sdd" in content
    ...
    def test_skills_install_for_all_agents(self, temp_dir, agent_key):
        ...
        assert skills_dir.exists()
        skill_dirs = [d.name for d in skills_dir.iterdir() if d.is_dir()]
        # Kimi uses dot-separator (sdd.specify) to match /skill:sdd.* invocation;
        # all other agents use hyphen-separator (sdd-specify).
        expected_skill_name = "sdd.specify" if agent_key == "kimi" else "sdd-specify"

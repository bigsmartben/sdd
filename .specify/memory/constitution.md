...
- **Rule**: Repository-first analysis serves only two semantic conclusions: technical dependency matrix and invocation/layering governance.
...
- **Invocation projection binding (hard rule)**:
...
- **Downstream artifact separation (hard rule)**: `technical-dependency-matrix.md` (facts) and `module-invocation-spec.md` (execution constraints) are complementary projections and MUST NOT replace one another.

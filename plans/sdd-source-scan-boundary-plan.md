...
这次应基于现有的 [`Repo-Anchor Evidence Protocol`](.specify/memory/constitution.md:133) 做增强，而不是回到抽象的 [`Repository-First Principle`](templates/constitution-template.md:146) 去重新定义一层。

原因很明确：

- 当前宪法在 [`.specify/memory/constitution.md`](.specify/memory/constitution.md:135) 已经直接规定 repo semantic anchors 的来源边界
- 当前宪法在 [`.specify/memory/constitution.md`](.specify/memory/constitution.md:136) 已经存在 whitelist 结构
- 这次要修的不是 是否仓库优先，而是 仓库分析到底该扫描什么，才能得出正确的依赖矩阵、组件能力边界、源码目录结论

因此，正确方向应是：

- 在现有 [`Repo-Anchor Evidence Protocol`](.specify/memory/constitution.md:133) 中增强扫描目标与允许来源
- 下游 `sdd` 命令统一引用这个协议执行
...
在 [`Repo-Anchor Evidence Protocol`](.specify/memory/constitution.md:133) 下明确：
...
- [`.specify/memory/constitution.md`](.specify/memory/constitution.md) 仅作为规则来源，不作为组件能力边界的源码证据
...
- [ ] 以现有 [`Repo-Anchor Evidence Protocol`](.specify/memory/constitution.md:133) 为承接点，增强其扫描目标定义
...
- 宪法承接点改成 [`Repo-Anchor Evidence Protocol`](.specify/memory/constitution.md:133)

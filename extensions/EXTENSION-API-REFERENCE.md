...
- **Format**:  `sdd.{extension-id}.{command-name}`
- **Examples**: `sdd.jira.specstoissues`, `sdd.linear.sync`
- **Invalid**: `jira.specstoissues`, `sdd.command`, `sdd.jira.CreateIssues`
...
```json
{
  ...
  "extensions": {
    "jira": {
      ...
      "enabled": true,
      "registered_commands": ["sdd.jira.specstoissues", ...],
...
.claude/
└── commands/
    └── sdd.{ext}.{cmd}.md  # Registered commands
```
